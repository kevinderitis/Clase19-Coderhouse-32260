...
mongoose.connect('mongodb+srv://coderhouse:coder123456@codehouse.wecfmdo.mongodb.net/coderhouse?retryWrites=true&w=majority')
...
authRouter.post('/restaurarPassword', sessionValidation,  async (req, res) => {
    ...
    try {
        ...
        if(!userFound){
            ...
            let result = await usuarioModel.updateOne({ email: user.email }, { $set: { password: newPassword }})
            res.render('login', {})
        }
